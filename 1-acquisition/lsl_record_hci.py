...
default_fname = ("../../data/hci_exp/data_%s.csv" % strftime("%Y-%m-%d-%H.%M.%S", gmtime()))
parser = OptionParser()
...
ch_names = [F3, F8, F7, F4, P3, P4, T4, T3, Fp2, Fp1, C4, C3, T6, T5, O1, O2]
print('Chanel check:', ch_names)
    ...
    try:
        ...
        if inlet_marker:
            ...
            if timestamp:
                ...
                print(marker)
        if len(res) in range(0, 1000000, 100) :
            print("Processed  %d records" %(len(res)*8)) 
