...
for i,path_task_type in enumerate(path_task_types):
    path = "../data/load_as_pd/round2/par1/{file}{path_task_type}.pkl".format( file = file, path_task_type = path_task_type)
    f = open(path,'rb')
    df = pickle.load(f)
...
#         psd = psd.mean(axis=1)
        X.append(psd)
    X = np.array(X)
    np.save("../data/preprocessed_np/round2/freq/{drift}/{file}_{task_type}_X".format(drift = drift, file=file, task_type = task_type), X)
    np.save("../data/preprocessed_np/round2/freq/{drift}/{file}_{task_type}_y".format(drift = drift, file=file, task_type = task_type), y)
