...
X = np.load('../data/preprocessed_np/round2/freq/{drift}/{file}_{task}_X.npy'.format(drift = drift,file=file, task=task), allow_pickle=True)
y = np.load('../data/preprocessed_np/round2/freq/{drift}/{file}_{task}_y.npy'.format(drift = drift,file=file, task=task), allow_pickle=True)
...
for i, model in enumerate(models):
    ...
    for epoch in range(N_EPOCHS):
        ...
        if valid_loss < best_valid_loss:
            ...
            torch.save(model.state_dict(), "../model/feature_extraction/round2/freq/{model_name}/{drift}/EEG_ENCODER_{task}.pt.tar".format(model_name=model_name,drift=drift,task=task))
            best_model_index = i
...
model = model.float()
model = model.to(device)
model.load_state_dict(torch.load('../model/feature_extraction/round2/freq/{model_name}/{drift}/EEG_ENCODER_{task}.pt.tar'.format(model_name=model_name,drift=drift,task=task)))
...
# 9. SAVE
# Save Real Test
np.save("../data/participants/freq/{task}/X_real_test".format(task=task),X_real_test)
np.save("../data/participants/freq/{task}/y_real_test".format(task=task),y_filled_real_test)

# Save Train
np.save("../data/participants/freq/{task}/X_train".format(task=task),X_train)
np.save("../data/participants/freq/{task}/y_train".format(task=task),y_train)

# Save Test
np.save("../data/participants/freq/{task}/X_test".format(task=task),X_test)
np.save("../data/participants/freq/{task}/y_test".format(task=task),y_test)

# Save Val
np.save("../data/participants/freq/{task}/X_val".format(task=task),X_val)
np.save("../data/participants/freq/{task}/y_val".format(task=task),y_val)

# Save Extracted Features
np.save('../data/participants/freq/{task}/extracted_features_X'.format(task=task), eeg_extracted_features )
np.save('../data/participants/freq/{task}/extracted_features_y'.format(task=task), y_train_val)


# 10. Results
# with open(f"../results/classification_results_freq_{sys.argv[3]}.txt", "a") as myfile:
...
with open(f"../results/classification_results_freq_{sys.argv[3]}.csv", "a") as myfile:
    task = sys.argv[3]
    myfile.write(f"{fmin}, {fmax},{sys.argv[1]},{train_acc},{valid_acc},{test_acc},{real_test_acc},")
    for v,k in acc_class_test.items():
        myfile.write(f"{k[0]},")
    for v,k in acc_class_real_test.items():
        myfile.write(f"{k[0]},")
