...
for i in range(54):
    num = i
    img = plt.imread('../data/participants/{par}/{task}/FID/{test_type}/image_{num}.png'.format(par=par,task=task,test_type=test_type,num=num))
    imgs.append(img)
...
for idx, val in enumerate(label_imgs):
    img_label = Image.open('../2-HCI_stimuli/stimulus/cylinder/{}/{}.png'.format(val,idx+1))
...
label = np.load('../data/participants/{par}/{task}/FID/{test_type}/generated_labels.npy'.format(par=par,task=task,test_type=test_type))
